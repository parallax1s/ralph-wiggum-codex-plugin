#!/usr/bin/env bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
"${REPO_ROOT}/tests/test-ralph-plugin-bundle.sh"
"${REPO_ROOT}/tests/test-ralph-status.sh"
"${REPO_ROOT}/tests/test-ralph-loop.sh"
bash "${REPO_ROOT}/tests/test-session-start-bridge.sh"
bash "${REPO_ROOT}/tests/test-session-start-bridge-concurrency.sh"
python3 -m unittest discover "${REPO_ROOT}/local/tests"
python3 -m unittest "${REPO_ROOT}/tests/test-ralph-arm-visible-thread.py"
python3 -m unittest "${REPO_ROOT}/tests/test-ralph-visible-thread.py"
"${REPO_ROOT}/tests/test-install-codex.sh"
echo "All Ralph Wiggum Codex Plugin tests passed."
